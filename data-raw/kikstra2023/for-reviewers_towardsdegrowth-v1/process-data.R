#' Script for Kikstra et al. "Interpreting new post-growth scenarios"
#' Last updated: 27.06.2023
#' function:
#' - load and process li data
...
# Files: Li ====
LI.DATA.FILE <- "DegrowthIAM_AU_IAMC_20230528.xlsx"
...
# preprocess data ====
preprocess_data_mengyu <- function(fname){
  ...
  rd1.raw <- read_excel(
    ...
    mutate(region="AUS") %>% # to replace blanks if present in the data
    
    # STILL NECESSARY?
    # AD-HOC; see email 30.03.2023
  ...
  # delete all values that are zero but shouldn't be - uncomment to check whether anything is unexpected
  # d %>% unique_variable()
  # write_delim(
  #   x = d %>% group_by(model,scenario,region,variable,unit) %>%
  #     filter(any(value==0 & year>2020) | any(variable=="Useful Energy" & year==2020 & value==0)) %>%
  #     filter(!(variable%in%c("GHG emissions"))) %>% 
  #     filter(!grepl(x = variable, pattern = "Primary Energy|", fixed=T)) %>%
  #     filter(!grepl(x = variable, pattern = "Hydrogen", fixed=T)) %>%
  #     filter(!grepl(x = variable, pattern = "Carbon price", fixed=T)) %>%
  #     filter(!all(value==0)),
  #   file = here("output", "unexpected_zeros.csv"), 
  #   delim = ","
  # )
...
li.data <- preprocess_data_mengyu(fname = LI.DATA.FILE)
...
# filter region and category 
# this line is not necessary, as it is also done in the figure scripts.
# however, it was added here to save some space in the zip for reviewers.
ipcc.data <- ipcc.data %>% 
